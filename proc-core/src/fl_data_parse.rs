...
impl DataParser for DataParserFLab {
    fn files_to_fetch_list(
        ...
        result.insert(
            DataParserFLabFiles::Data.to_key(),
            format!("data/vendor/{}/data.json", conf.id),
        );
        result
    }
    ...
    fn parse(
        ...
        let parsed: FLabData =
            serde_json::from_str(jsons.get(&DataParserFLabFiles::Data.to_key()).unwrap()).unwrap();
...
impl FLabDataItem {
    fn new_factory_from(
        &self,
        fg_sets: &Vec<String>,
        fgs: &HashMap<String, Rc<FactoryGroup>>,
        ...
        Ok(Factory {
            ...
            groups: fg_sets
                ...
                .map(|v| -> Result<Rc<FactoryGroup>, String> {
                    Ok(fgs.get(&format!("fg-{}", v.0)).cloned().ok_or_else(|| {
                        format!("unable for find a factory group containing {}", self.id)
                    })?)
                })
    ...
    fn new_from(
        ...
        Ok(Process {
            ...
            inputs: self
                .input
                .iter()
                .map(|v| -> Result<Stack, String> {
                    Ok(Stack {
                        item: items
                            .get(v.0)
                            .cloned()
                            .ok_or_else(|| format!("can't find item {}", v.0))?,
                        quantity: v.1.clone(),
                    })
                })
...
mod test {
    ...
    fn create_input_fixture() -> BTreeMap<String, String> {
        ...
        jsons.insert(
            DataParserFLabFiles::Data.to_key().to_string(),
            load_fixture("fixtures/fl_data.json").to_string(),
        );
        jsons
    }
    ...
    fn it_understands_processes() {
        ...
        assert_eq!(
            process
                ...
                .sorted()
                .collect::<Vec<String>>(),
            &["technum-rod", "xenoferrite-plates"]
        );
        assert_eq!(
            process
                .inputs
