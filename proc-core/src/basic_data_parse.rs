...
impl DataParser for DataParserBasic {
    fn files_to_fetch_list(&self, conf: &DataSetConf) -> BTreeMap<String, String> {
        ...
        result.insert(
            DataParserBasicFiles::Main.to_key(),
            format!("data/{}.json", conf.id),
        );
        result
    }
    ...
    fn parse(&self, jsons: &mut BTreeMap<String, String>) -> Result<Data, String> {
        ...
        let items_res: Result<Vec<Rc<Item>>, String> = outer["items"]
            .as_array()
            .ok_or("missing '.items'")?
            .iter()
            .enumerate()
            .map(|(idx, i)| {
                let id = i["id"]
                    .as_str()
                    .ok_or(format!("missing '.items[{idx}].id'"))?
                    .to_string();
                Ok(Rc::new(Item {
                    id: id.clone(),
                    display: i["i18n"]["en"]
                        .as_str()
                        .ok_or(format!("missing '.items[{idx}].i18n.en' (id: {id})"))?
                        .to_string(),
                    classification: Classification::Solid,
                }))
            })
            .collect();
        let items: HashMap<String, Rc<Item>> =
            items_res?.into_iter().map(|i| (i.id.clone(), i)).collect();

        let mut factory_groups: HashMap<String, Rc<FactoryGroup>> = HashMap::new();
        let mut factories: HashMap<String, Rc<Factory>> = HashMap::new();

        for (idx, fac) in outer["factories"]
            .as_array()
            .ok_or("missing '.factories'")?
            .iter()
            .enumerate()
        {
            let id = fac["id"]
                .as_str()
                .ok_or(format!("missing '.factories[{idx}].id''"))?
                .to_string();
            for (fg_id, fac_grp) in fac["factory_groups"]
                .as_array()
                .ok_or(format!(
                    "missing '.factories[{idx}].factory_groups' (id: {id})"
                ))?
                .iter()
                .enumerate()
            {
                let id = fac_grp
                    .as_str()
                    .ok_or(format!(
                        "unparsable '.factories[{idx}].factory_groups[{fg_id}]' (id: {id})"
                    ))?
                    .to_string();
            ...
            factories.insert(
                ...
                Rc::new(Factory {
                    ...
                    display: fac["i18n"]["en"]
                        .as_str()
                        .ok_or(format!("missing '.factories[{idx}].i18n.en' (id: {id})"))?
                        .to_string(),
                    groups: fac["factory_groups"]
                        .as_array()
                        .ok_or(format!("missing '.items[{idx}].factory_groups' (id: {id})"))?
                        .iter()
        ...
        for (idx, proc) in outer["processes"]
            .as_array()
            .ok_or("missing '.processes'")?
            .iter()
            .enumerate()
        {
            let id = proc["id"]
                .as_str()
                .ok_or(format!("missing '.processes[{idx}].id'"))?
                .to_string();
