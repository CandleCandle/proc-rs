...
use crate::{
    ...
    model::{Classification, Data, Factory, FactoryGroup, Item, Process, Stack},
};
...
pub fn simple_data_fixture() -> Data {
    ...
    factories.insert(
        "just_basic2".to_string(),
        Rc::new(Factory {
            id: "just_basic2".to_string(),
            display: "just_basic2".to_string(),
            groups: vec![factory_groups.get(&"basic2".to_string()).unwrap().clone()],
    ...
    let p1 = Process {
        ...
        outputs: vec![Stack::new(items.get("part_3").unwrap().clone(), 5.0)],
    ...
    let p2 = Process {
        ...
        duration: 1.0,
        group: factory_groups.get(&"basic".to_string()).unwrap().clone(),
        inputs: vec![Stack::new(items.get("part_3").unwrap().clone(), 3.0)],
        inputs_unmod: Vec::new(),
        outputs: vec![
    ...
    let p3 = Process {
        ...
        duration: 1.0,
        group: factory_groups.get(&"basic".to_string()).unwrap().clone(),
        inputs: vec![Stack::new(items.get("part_1").unwrap().clone(), 5.0)],
        inputs_unmod: Vec::new(),
        outputs: vec![Stack::new(items.get("part_2").unwrap().clone(), 1.0)],
        outputs_unmod: Vec::new(),
    };
    let p4 = Process {
    ...
    let processes = vec![p1, p1_slow, p2, p3, p4]
        .iter()
...
pub fn create_config() -> GraphConfiguration {
    ...
    gc.set_data(
        ...
        DataSetConf {
            id: "".to_string(),
            style: "basic".to_string().try_into().unwrap(),
        },
    );
    gc
}
