...
use proc_core::{
    ...
    dataset::DataSetConf,
    graph_configuration::{DehydratedGraphConfiguration, FetchDataSet, GraphConfiguration},
...
impl FetchDataSet for FileFetcher {
    async fn fetch(&self, relative_path: &str) -> Result<String, String> {
        ...
        let mut file = File::open(
            PathBuf::from_str(&self.base_dir)
                .unwrap()
                .join(relative_path),
        )
        .map_err(|e| format!("{} ({})", e, relative_path))?;
...
async fn generate(args: GenerateArgs) -> Result<(), String> {
    ...
    for req in args.requirements {
        let parts = req.split_once(':').ok_or(format!(
            "requirements must be in the form F:id, found {req}"
        ))?;
    ...
    for proc in args.processes {
        ...
        gc.add_process(
            id,
            gc.get_fastest_factory_for_process(id).as_str(),
            dur_mod,
            in_mod,
            out_mod,
        );
...
fn report_gc(gc: GraphConfiguration, graph: Option<PathBuf>) -> Result<(), String> {
    ...
    tracing::info!(
        ...
        serde_json::to_string(&gc.dehydrate()).unwrap()
    );
    tracing::info!(
        "serialised configuration {}",
        BASE64_STANDARD_NO_PAD.encode(rmp_serde::encode::to_vec(&gc.dehydrate()).unwrap())
    );
    ...
    if graph.is_some() {
        fs::write(graph.unwrap(), calc.to_digraph()).map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
fn make_materials_count_table(materials: &StackSet) -> String {
    ...
    for item in &all_items {
        ...
        if net.is_sign_negative() && net.abs() < 0.001 {
            // essentially, display -0 as 0.
            net = -net;
        }
